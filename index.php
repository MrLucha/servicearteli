...
function scrapProducts($link){
    ...
    $buscador = $html->find('div[class=body]');
    foreach ($buscador as $b) {
        $prices=$b->find('.price-current');
        foreach ($prices as $p) {
            $enteros = $p->find('span[class=value]');
            foreach ($enteros as $e) {
                $price=$price.$e->text();
            }
            $centavos = $p->find('span[class=sub-price]');
            foreach ($centavos as $c) {
                $price=$price.$c->text();
                $eliminar = array("\t", "$", " ");
                $precio_producto = str_replace ( $eliminar, '', $price);
